...
\tikzset{resistor label/.style={l=$R_{s}$}}
...
% resistor
\draw[line style] (A2) to[R, l=$R_{s}$, fill=black] (C1);

% inductor
\ctikzset{inductor=american}
\draw[line style] (C1) to[L, l=$L_{s}$] (C2);

% capacitor
\draw[line style] ($(A2) - (2pt, 0)$) to[C, l=$C_{s}$] ($(B2) - (2pt, 0)$);

% G
\draw[line style] (C2) to[R, l=$G_{s}$] (D2);
...
\draw[{Circle[open, fill=white]}-{Circle[open, fill=white]}, line style]  ($(C2) - (2pt, 0)$) -- (E1);

\draw[-{Circle[open, fill=white]}, line style] (D2) -- (E2);
\draw[-{Circle[open, fill=white]}, line style] (B2) -- ($(D2) + (2pt, 0)$);

\draw[Stealth-Stealth] (F1) -- (F2) node[pos=0.5, circle, fill=white, below] {$s$};
