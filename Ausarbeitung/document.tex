...
wie es insbesondere bei hochfrequenten Signalen der Fall ist. Das Fließen eines zeitlich veränderlichen elektrischen
Stroms in einem Leiter induziert die Ausbildung elektromagnetischer Felder, die i.\,A. durch die Maxwellschen
Gleichungen beschrieben werden. Die Methode der Ersatzschaltbilder bietet einen einfacheren Zugang zur Ableitung der
Gleichungen des elektrischen Leiters mittels der Gesetzt der Elektro- und Magnetostatik. Sie basieren wesentlich auf
die Anwendung der Kirchhoffschen Maschen- und Knotengleichungen.
...
\begin{align}
u(x, t) &= u_{h}(v t - x) + u_{r}(v t + x) \label{eq:AllgEq1} \\[1ex]
i(x, t) &= i_{h}(v t - x) + i_{r}(v t + x) \label{eq:AllgEq2}.
\end{align}
